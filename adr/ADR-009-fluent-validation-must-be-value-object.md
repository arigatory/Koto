...
В проектах Koto применяется подход **«Always Valid Domain Model»**: вся логика валидации инкапсулирована в фабричных методах доменных объектов (`Email.Create`, `OrderId.Create` и т. д.). Метод фабрики возвращает `Result<TValueObject, Error>` — либо корректно созданный объект-значение, либо структурированную ошибку.
...
Дополнительная проблема: **FluentValidation v8+ переходит на коммерческую лицензию** (Xceed). Версия v7.x выпущена под лицензией Apache 2.0 и останется под ней навсегда — смена лицензии не распространяется на уже выпущенные версии. Переход на v8+ означал бы необходимость приобретения коммерческой лицензии для использования в закрытых коммерческих проектах.
...
| Поддержка объектов-значений | Метод `MustBeValueObject` должен работать с любым фабричным методом сигнатуры `Func<string, Result<TValueObject, Error>>` |
...
|---|---|---|
| Лицензионная чистота | Используемая версия FluentValidation должна оставаться под Apache 2.0 навсегда | Обязательно |
...
|---|---|
| Лицензионная безопасность | FluentValidation v7.x выпущен под Apache 2.0 и не может быть ретроактивно перелицензирован. Фиксация на v7 исключает непреднамеренный переход на коммерческую v8+ |
...
- FluentValidation v7 остаётся под Apache 2.0 — никаких лицензионных рисков
...
|---|---|---|---|
| **FluentValidation v8+ (коммерческая)** | Актуальная версия, новые возможности, долгосрочная поддержка от Xceed | Требует коммерческой лицензии для закрытых проектов; дополнительные расходы; риск vendor lock-in | Лицензионные затраты неоправданны: v7 полностью закрывает потребности, а паттерн `MustBeValueObject` работает с v7 идеально |
