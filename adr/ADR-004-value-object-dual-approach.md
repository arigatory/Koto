...
| Всегда валидное состояние | Создание VO с невалидными данными должно быть невозможным; фабричный метод возвращает `Result<T, Error>` |
...
| Единый паттерн валидации | Оба подхода используют `private constructor + static Create → Result<T, Error>`, что соответствует принципу «always valid domain model» (Хориков) |
...
- Паттерн `Result<T, Error>` (библиотека ошибок) должен быть доступен до реализации VO
...
3. **Дивергенция двух подходов со временем**  
   ...
   *Меры:* Поддерживать единый шаблон `Create → Result<T, Error>` как обязательный для обоих подходов; регулярно проводить ревью существующих VO на соответствие паттерну.
